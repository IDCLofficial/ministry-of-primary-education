...
- **Status**: ✅ Active
- **Portal**: `/result-checking/dashboard?exam=bece`
- **Features**: 
  - View results
...
- **Status**: 🔜 Coming Soon
- **Portal**: `/result-checking/dashboard?exam=ubeat`
- **Features**: Will be similar to BECE portal

## Key Files

### 1. `/result-checking/page.tsx` (Landing/Exam Selection Page)
...
### 2. `/result-checking/bece/page.tsx` (BECE Login Page)
...
| `student-payment-return-url` | Payment callback URL | `/result-checking/dashboard` |
...
const examTypes: ExamType[] = [
    // ... existing exam types ...
    {
        ...
        route: '/result-checking/new-exam'
    }
]
```
...
Create `/result-checking/new-exam/page.tsx`:

```typescript
// Copy from /result-checking/bece/page.tsx and modify:
...
Edit `/result-checking/dashboard/page.tsx` to handle the new exam type:
...
If the new exam has different API endpoints, update `/result-checking/utils/api.ts`:
...
- `/result-checking/dashboard/components/NewExamResultsCard.tsx`
- `/result-checking/dashboard/components/NewExamInfoCard.tsx`

### 6. Test the Flow

```
1. Visit /result-checking (landing)
2. Select new exam type
3. Verify redirect to /result-checking/new-exam
4. Enter exam number
...
- User is redirected to `/result-checking/select-exam`
...
- User is redirected to `/result-checking` (login)
...
1. **Login Test**
   ```
   Navigate to /result-checking
   Enter valid exam number
   Verify redirect to /result-checking/select-exam
   ```
