...
#### 3. Update `src/app/result-checking/ubeat/page.tsx`
...
export default function UBEATLoginPage() {
    ...
    const onSuccess = (reference: any) => {
        ...
        router.push(`/result-checking/ubeat/payment?ref=${reference.reference}&status=success`);
    };
...
#### 4. Update `src/app/result-checking/ubeat/payment/page.tsx`
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    setTimeout(() => {
                        ...
                        router.push('/result-checking/ubeat/dashboard')
                    }, 2000)
                } else {
                    setPaymentStatus('failed')
                }
        ...
        handleFlutterPayment({
            callback: (response) => {
                ...
                if (response.status === 'successful') {
                    ...
                    localStorage.setItem('ubeat_alt_form_data', JSON.stringify(altFormData));
                    router.push(`/result-checking/ubeat/payment?ref=${response.transaction_id}&status=success`);
                }
                closePaymentModal();
            },
            onClose: () => {
