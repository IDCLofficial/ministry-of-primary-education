...
export default function BECEVerifyPage() {
    ...
    const loginUrl = useMemo(() => {
        if (!payload?.examNo) return '/student-portal/bece'
        return `/student-portal/bece?examNo=${encodeURIComponent(payload.examNo)}`
    }, [payload?.examNo])
        ...
        return (
            <div className="min-h-screen bg-gradient-to-b from-white to-emerald-50/30 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 max-w-md w-full text-center">
                    ...
                    <Link href="/student-portal/bece" className="text-green-600 font-medium hover:underline">
                        Go to BECE login
                    </Link>
                </div>
            </div>
