          ...
          {availableExams.length > 0 ? (
            availableExams.map((exam) => (
              exam.isAvailable ? (
                <Link
                  key={exam.key}
                  href={`/exam-portal/dashboard/uploads/${exam.key}`}
                ...
                <div
                  key={exam.key}
                  className="flex items-center gap-3 px-3 py-2.5 opacity-50 cursor-not-allowed border-l-2 border-transparent"
                >
                  <Image
                    src={exam.iconPath}
                    alt={exam.shortName}
