...
export default function ExamModal({ isOpen, onClose, student, onUpdate }: ExamModalProps) {
  ...
  const getScoreColor = (score: number) => {
    // Exam scores are out of 70, so 49/70 (70%) is excellent, 35/70 (50%) is good
    if (score >= 49) return 'bg-green-100 text-green-800 border-green-200'
    if (score >= 35) return 'bg-yellow-100 text-yellow-800 border-yellow-200'
    return 'bg-red-100 text-red-800 border-red-200'
  }

  const getPercentage = (score: number) => {
    return ((score / 70) * 100).toFixed(1)
  }
  ...
  const handleScoreChange = (subjectKey: string, value: string) => {
    ...
    setEditedStudent({
      ...editedStudent,
      [subjectKey]: Math.min(Math.max(numValue, 0), 70) // Clamp between 0 and 70
    })
  }
