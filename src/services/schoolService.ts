...
export const fetchAllPayments = async () => {
  const res = await fetch(`${API_BASE_URL}/student-payments/all`,
    {
      method: "GET",
      cache: "no-store"
    }
...
export const fetchSchoolById = async (schoolId: string): Promise<School> => {
  try {
    ...
    const apiResponse: ApiResponse<SchoolApiData> = await response.json()
    
    if (!apiResponse.data) {
      throw new Error('School not found')
    }

    return transformSchoolData(apiResponse.data)
  } catch (error) {
